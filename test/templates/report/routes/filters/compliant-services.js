...
describe('Routes: Filters -> Compliant services', () => {
  it('should have a get function: /filters/compliant-services', () => {
    const router = require(path.resolve('src', 'templates', 'report', 'pages', 'filters', 'compliant-services', 'routes'))
  ...
  it('should render a template: /filters/compliant-services', () => {
    ...
    cleanDataModel()
    buildDataModel('test')
    const { get } = require(path.resolve('src', 'templates', 'report', 'pages', 'filters', 'compliant-services', 'functions'))
    const req = {}
    const res = { send: (html) => { } }
    sinon.spy(res, 'send')
