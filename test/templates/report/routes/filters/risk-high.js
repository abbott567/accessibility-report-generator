...
describe('Routes: Filters -> High risk services', () => {
  it('should have a get function: filters/high-risk-services', () => {
    const router = require(path.resolve('src', 'templates', 'report', 'pages', 'filters', 'high-risk-services', 'routes'))
  ...
  it('should render a template: filters/high-risk-services', () => {
    ...
    cleanDataModel()
    buildDataModel('test')
    const { get } = require(path.resolve('src', 'templates', 'report', 'pages', 'filters', 'high-risk-services', 'functions'))
    const req = {}
    const res = { send: (html) => { } }
    sinon.spy(res, 'send')
