...
describe('Routes: Filters -> Critical services', () => {
  it('should have a get function: /filters/critical-services', () => {
    const router = require(path.resolve('src', 'templates', 'report', 'pages', 'filters', 'critical-services', 'routes'))
  ...
  it('should render a template: /filters/critical-services', () => {
    ...
    cleanDataModel()
    buildDataModel('test')
    const { get } = require(path.resolve('src', 'templates', 'report', 'pages', 'filters', 'critical-services', 'functions'))
    const req = {}
    const res = { send: (html) => { } }
    sinon.spy(res, 'send')
