...
describe('Routes: Filters -> Staff facing services', () => {
  it('should have a get function: /filters/staff-facing-services', () => {
    const router = require(path.resolve('src', 'templates', 'report', 'pages', 'filters', 'staff-facing-services', 'routes'))
  ...
  it('should render a template: /filters/staff-facing-services', () => {
    ...
    cleanDataModel()
    buildDataModel('test')
    const { get } = require(path.resolve('src', 'templates', 'report', 'pages', 'filters', 'staff-facing-services', 'functions'))
    const req = {}
    const res = { send: (html) => { } }
    sinon.spy(res, 'send')
