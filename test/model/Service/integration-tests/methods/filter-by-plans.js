...
describe('Int: Service methods -> filterByPlans()', () => {
  it('should return services with plans using filterByPlans(true)', () => {
    // Set up Org
  ...
  it('should return services with no plans using filterByPlans(false)', () => {
    // Set up Org
    const orgParams = cloneDeep(testOrg)
    const org = new Org(orgParams)
    org.save()
    ...
    service2.save()
    // Test
    const filteredServices = Service.filterByPlans('false')
    expect(filteredServices).to.include(service1)
    expect(filteredServices).to.not.include(service2)
    // Cleanup
    Org.all = []
