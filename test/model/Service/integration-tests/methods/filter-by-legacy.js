...
describe('Int: Service methods -> filterByLegacy()', () => {
  it('should return legacy services using using filterByLegacy(true)', () => {
    // Set up Org
    ...
    service2.save()
    // Test
    const filteredServices = Service.filterByLegacy('true')
  ...
  it('should return non-legacy services using using filterByLegacy(false)', () => {
    // Set up Org
    const orgParams = cloneDeep(testOrg)
    const org = new Org(orgParams)
    org.save()
    ...
    service2.save()
    // Test
    const filteredServices = Service.filterByLegacy('false')
    expect(filteredServices).to.include(service1)
    expect(filteredServices).to.not.include(service2)
    // Cleanup
    Org.all = []
